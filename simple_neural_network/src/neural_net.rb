...
class NeuralNet
  ...
  def initialize_new_weights
    init_factor_1 = 0.01 / ((IMG_AREA + 1) ** (0.5))    # weight scaler for the first weight matrix. It is indirectly proportional to the input size. It has to be, otherwise you will blow up your hidden layer. If you do not put in the deflator. The first couple of itterations will work on large sums in the hidden layer. Large sums are bad in neural networks.
    init_factor_2 = 0.01 / (hidden_nodes ** (0.5))
    
    @w1 = (NMatrix.random([IMG_AREA + 1, hidden_nodes]) - (NMatrix.ones([IMG_AREA + 1, hidden_nodes]) /2)) * init_factor_1 # some basic matrix algebra, create a matrix with
    @w2 = (NMatrix.random([hidden_nodes + 1, CLASSES_NUMBER])  - (NMatrix.ones([hidden_nodes + 1, CLASSES_NUMBER]) / 2)) * init_factor_2
  end
